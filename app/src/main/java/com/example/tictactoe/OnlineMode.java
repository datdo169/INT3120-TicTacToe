...
public class OnlineMode extends AppCompatActivity {
    ...
    private final List<String> doneBoxes = new ArrayList<>(); //done boxes position by users so users won't select the box again
    // player unique Id
    ...
    DatabaseReference databaseReference = FirebaseDatabase.getInstance().getReferenceFromUrl("https://test-6ecea-default-rtdb.firebaseio.com/");
    ...
    //connection id in which players has joined to play the game
            ...
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                if (snapshot.hasChild("player_id")) {
                    ...
                    if (getWinPlayerId.equals(playerUniqueId)) {
                        winDialog = new WinDialog(OnlineMode.this, "You won the game");
                    } else {
                        winDialog = new WinDialog(OnlineMode.this, "Opponent won the game");
                    }
    ...
    private void selectBox(ImageView imageView, int selectedBoxPosition, String selectedByPlayer) {
        ...
        if (doneBoxes.size() == 9) {
            final WinDialog winDialog = new WinDialog(OnlineMode.this, "It is a Draw!");
            winDialog.setCancelable(false);
            winDialog.show();
        }
    }
